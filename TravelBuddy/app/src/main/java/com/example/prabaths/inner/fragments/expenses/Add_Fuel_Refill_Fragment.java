...
public class Add_Fuel_Refill_Fragment extends Fragment {
    ...
    private void submit(){
        if(!Home_Fragment.regNo.equals("")) {
            if (validateForm()) {
                SQLiteHelper helper = SQLiteHelper.getInstance(getContext());
                Date date = null;
                boolean validOdometerReading=true;
                try {
    ...
    private void showTurnOnInternetAlertDialog(){
        AlertDialog.Builder alertDialog = new AlertDialog.Builder(getContext());
        ...
        // Setting Dialog Message
        alertDialog.setMessage("Internet is disabled. Please turn on internet. If you want to proceed without internet, select Proceed!!!");
